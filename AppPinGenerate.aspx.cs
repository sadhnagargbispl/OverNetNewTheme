...
public partial class AppPinGenerate : System.Web.UI.Page
{
    SqlConnection Conn;
    SqlCommand Comm;
    DAL ObjDAL;
    ...
    private void generatepin()
    {
        ...
        if (Convert.ToDouble(Session["Balance"]) >= TotalAmount)
        {
            ...
            if (i > 0)
            {
                ...
                scrname = "<SCRIPT language='javascript'>alert('Pin Sucessfully Generate.!');location.replace('AppPinGenerate.aspx');</SCRIPT>";
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Upgraded", scrname, false);
                return;
            }
            else
            {
                scrname = "<SCRIPT language='javascript'>alert('Try Later.!');location.replace('AppPinGenerate.aspx');</SCRIPT>";
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Upgraded", scrname, false);
                return;
            }
        }
