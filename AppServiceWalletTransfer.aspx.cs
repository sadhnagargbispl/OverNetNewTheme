...
public partial class AppServiceWalletTransfer : System.Web.UI.Page
{
    string scrName;
    DAL objDal = new DAL();
        ...
        try
        {
            ...
            if (Dt1.Rows.Count > 0)
            {
                ...
                string R = "update adminlogin SET Expired = 1 where ForType = 'APP' AND MobileNo = '" + mobileno + "' AND Expired = '0';" +
                           "INSERT into AdminLogin(UserID,Username,Passw,MobileNo,Otp,Expired,ForType) VALUES ('0','" + membername + "','0','" + mobileno + "','" + OTP_ + "','0','APP')";
                i = objDal.SaveData(R);
                if (i > 0)
                {
                    divOtp.Visible = true;
    ...
    public bool validotp(string username, string userMobile, string otpcode)
    {
        try
        {
            ...
            string strr = objDal.Isostart + "Select TOP 1 * from " + objDal.dBName + "..AdminLogin as a where " +
                          "UserName='" + username + "' and Mobileno='" + Convert.ToInt64(userMobile) + "' AND OTP = '" + otpcode + "' AND Expired = 0 AND ForType = 'APP' " +
        ...
        if (Convert.ToDecimal(Session["ServiceWallet"]) >= Convert.ToDecimal(txtAmount.Text))
        {
            ...
            if (Convert.ToDecimal(Session["ServiceWallet"]) >= Convert.ToDecimal(txtAmount.Text))
            {
                if (objDal.SaveData(query) != 0)
                {
                    ...
                    string message = "Amount Transfer Successfully.!";
                    string url = "AppServiceWalletTransfer.aspx";
